...
async function rodar() {
  ...
  const processAllClients = async (potenciais, authorize) => {
    const chunkSize = 60;
    for (let i = 0; i < potenciais.length; i += chunkSize) {
      const chunk = potenciais.slice(i, i + chunkSize);
      const results = await Promise.all(
        chunk.map(async (client) => {
          try {
            return await processClient(client, authorize);
          } catch (error) {
            if (error.message.includes("60 requests per minute"))
              await sleep(6000 * 5);
            if (error.response.status == 401 || error.response.status == 403) {
              try {
                ...
                if (err.message.includes("60 requests per minute"))
                  await sleep(6000 * 5);
                clientError.push({ data: client, error: err.response.message });
              }
            } else {
              clientError.push({ data: client, error: error.response.message });
      ...
      potenciaisClients.push(...results.flat().filter((f) => f != null));
      await sleep(6000);
    }
  };
  // if (request < 60) {
  ...
  // }
  // request++;
  // const results = await Promise.all(potenciais.map(client => ));
  ...
  const processList = async (lista) => {
    ...
    return (potenciais?.prospects ?? []).filter((f) => f.id);
  };
  ...
  const processAllLists = async (listas) => {
    const chunkSize = 60;
    for (let i = 0; i < listas.length; i += chunkSize) {
      const chunk = listas.slice(i, i + chunkSize);
      const results = await Promise.all(
        chunk.map(async (x) => {
          try {
            return await processList(x);
          } catch (error) {
            if (error.message.includes("60 requests per minute"))
              await sleep(6000 * 5);
            if (error.response.status == 401 || error.response.status == 403) {
      ...
      prospects.push(...results.flat());
      await sleep(6000);
    }
  };
  const processClientDetails = async (client) => {
    try {
      let tentativas = 0;
      let emails;
      ...
      const site = client.currentJob[0].site.split("/")[2];
      if (!site) return;
      ...
      while (emails.status.identifier !== "complete" && tentativas < 3) {
        await sleep(6000);
        emails = await listarEmails(
          authorize,
          client.firstName,
      ...
      if (tentativas >= 3 || !emails?.data?.emails[0]?.email) {
        return;
      }

      client.email = emails.data.emails[0].email;
      const base = await ConsultarCliente(client.email);
      const criticas = await Criticas(base, client);

      if (!criticas.length) {
        if (!base) {
          const ins = await InseirNaBase(client);
        } else {
          client.id = base.id;
          const up = await AtualizarBase(client);
        }
      } else {
        console.warn("-----------------------------------------------");
        console.warn(client.email);
        criticas.forEach((x) => console.log(x));
        console.warn("-----------------------------------------------");
      }
    } catch (err) {
      console.error(err);
  ...
  await processAllLists(listas);

  await Promise.all(
    prospects.map(async (prospect) => {
      try {
        await await prisma.bigProspect.create({
          data: {
  ...
  // await sleep(60000 * 3);
  // await processAllClients(prospects);
  // await sleep(60000 * 2);
  // await Promise.all(potenciaisClients.map(processClientDetails));
